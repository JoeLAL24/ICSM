...
class GroupAttention(nn.Module):
    ...
    def forward(self, feas, ds=False, scale=0.5):
        # feas:8*512*16*16
        bs, _, h, w = feas.shape # bs=8, h=16, w=16
        if ds:
            ...
            query_feas = rearrange(feas, "b c h w -> (b h w) c").unsqueeze(0) #升维之后： 1*2048*512
        pooled_feas = self.msp_block(feas) #[1, 3, 6]多尺度池化，8*512*46（b=8,c=512,n=46）
        pooled_feas = rearrange(pooled_feas, "b c n -> (b n) c").unsqueeze(0) #升维之后： 1*368*512
        attn_out = self.attention(q=query_feas, k=pooled_feas, v=pooled_feas) #attn_out: 1*2018*512
        if ds:
            ...
            attn_out = rearrange(attn_out.squeeze(), "(b h w) c -> b c h w", b=bs, h=h) #attn_out:8*512*16*16
        feas = feas + self.drop_path(attn_out) #feas:8*512*16*16
        feas = rearrange(feas, "b c h w -> (b h w) c", b=bs, h=h).unsqueeze(0) #feas:1*2018**512
        if self.ffn is not None:
            feas = feas + self.drop_path(self.ffn(feas))
        feas = rearrange(feas.squeeze(), "(b h w) c -> b c h w", b=bs, h=h)#feas:8*512*16*16
        return feas
